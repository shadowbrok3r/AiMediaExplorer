...
use egui_data_table::{
    viewer::{default_hotkeys, RowCodec, UiActionContext},
    RowViewer, UiAction,
};
...
use crate::{generate_image_thumb_data, generate_video_thumb_data, Thumbnail};
...
impl FileTableViewer {
    pub fn new(thumbnail_tx: Sender<Thumbnail>) -> Self {
        Self { 
...
impl RowViewer<Thumbnail> for FileTableViewer {
    ...
    fn num_columns(&mut self) -> usize { match self.mode { ExplorerMode::FileSystem => 8, ExplorerMode::Database => 10 } }

    fn column_name(&mut self, column: usize) -> std::borrow::Cow<'static, str> {
        match self.mode {
            ExplorerMode::FileSystem => ["", "Name", "Path", "Category", "Tags", "Modified", "Size", "Type"][column].into(),
            ExplorerMode::Database => ["", "Name", "Path", "Category", "Tags", "Modified", "Size", "Type", "Hash", "DB Created"][column].into(),
        }
    }
        ...
        match self.mode {
            ExplorerMode::FileSystem | ExplorerMode::Database => {
                ...
                row.filename.to_lowercase().contains(&f)
                    ...
                    || row.category.as_ref().map(|s| s.to_lowercase().contains(&f)).unwrap_or(false)
            }
        }
    }
    ...
    fn show_cell_view(&mut self, ui: &mut egui::Ui, row: &Thumbnail, column: usize) {
        match column {
            0 => { // Thumbnail (mode-agnostic currently)
                if row.file_type == "<DIR>" {
                    ui.add_sized(
                        ui.available_size(),
                        egui::Image::new(eframe::egui::include_image!("../../../../assets/Icons/folder.png"))
                    );
                    return;
                }
                    ...
                    if !self.thumb_cache.contains_key(&cache_key) {
                        if let Some(mut b64) = row.thumbnail_b64.clone() {
                            ...
                            if let Ok(decoded) = B64.decode(b64.as_bytes()) {
                                self.thumb_cache.insert(cache_key.clone(), Arc::from(decoded.into_boxed_slice()));
                            }
                        }
                    }
                    if let Some(bytes_arc) = self.thumb_cache.get(&cache_key) {
                        let img_src = ImageSource::Bytes { uri: Cow::from(format!("bytes://{}", cache_key)), bytes: egui::load::Bytes::Shared(bytes_arc.clone()) };
                        Image::new(img_src).show_loading_spinner(true).max_size(ui.available_size()).ui(ui); // .bg_fill(Color32::WHITE)
                    } else {
                        ui.label("❌");
                    }
                }
            ...
            2 => { // Path or DB key depending on mode
                match self.mode {
            ...
            4 => { // Tags as chips
                if row.tags.is_empty() { ui.label("-"); } else {
                    ui.horizontal(|ui| {
                        for tag in &row.tags {
                            let color = color_for_tag(tag);
                            let txt = format!("{}", tag);
                            let pad = egui::vec2(6.0, 2.0);
                            let galley = ui.painter().layout_no_wrap(txt.clone(), FontId::monospace(12.), Color32::WHITE);
                            let (rect, resp) = ui.allocate_exact_size(galley.rect.size() + pad * 2.0, egui::Sense::hover());
                            ui.painter().rect_filled(rect, 6.0, color);
                            ui.painter().galley(rect.min + pad, galley, Color32::WHITE);
                            let stroke = Stroke { width: 1.0, color: color.gamma_multiply(0.6) };
                            ui.painter().rect_filled(rect, 6.0, color);
            ...
            8 => { // Hash (DB mode only)
                if self.mode == ExplorerMode::Database { ui.label(row.hash.as_deref().unwrap_or("-")); }
            }
            9 => { // DB Created date (DB mode only)
                if self.mode == ExplorerMode::Database { use chrono::Datelike; let c = &row.db_created; ui.label(format!("{}/{}/{}", c.month(), c.day(), c.year())); }
            }
            _ => unreachable!(),
        };
    }
        ...
        match column {
            0 => {
                ...
                if row.thumbnail_b64.is_none() {
                    ...
                    if let Some(bytes_arc) = self.thumb_cache.get(&cache_key) {
                        let img_src = ImageSource::Bytes { uri: Cow::from(format!("bytes://{}", cache_key)), bytes: egui::load::Bytes::Shared(bytes_arc.clone()) };
                        Some(Image::new(img_src).bg_fill(Color32::WHITE).fit_to_exact_size(Vec2::splat(50.)).ui(ui)) // .paint_at(ui, rect);
                    } else {
                        Some(ui.label("❌"))
                    }
                }
            ...
            4 => { // Tags editable
                ...
                if resp.changed() { row.tags = tag_str.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect(); }
                Some(resp)
            }
            3 => { // Category
                let mut v = row.category.clone().unwrap_or_default();
                let resp = ui.text_edit_singleline(&mut v);
                if resp.changed() { row.category = if v.is_empty() { None } else { Some(v) }; }
                Some(resp)
            }
    ...
    fn new_empty_row(&mut self) -> Thumbnail { Thumbnail::default() }

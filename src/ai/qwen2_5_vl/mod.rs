...
fn default_image_size() -> usize { 896 }
...
pub struct Config {
    ...
    pub hidden_size: usize,
    #[serde(alias = "vision_start_token_id")]
    pub vision_start_token_id: u32,
    #[serde(alias = "vision_end_token_id")]
...
impl MultimodalRotaryEmbedding {
    ...
    fn apply_rotary_pos_emb(
        ...
        let cos = cos.unsqueeze(1)?.expand((b, h, l, d_half))?;
        let sin = sin.unsqueeze(1)?.expand((b, h, l, d_half))?;

        let split_halves = |x: &Tensor| -> Result<(Tensor, Tensor)> {
            let x1 = x.narrow(candle_core::D::Minus1, 0, d_half)?;
            let x2 = x.narrow(candle_core::D::Minus1, d_half, d_half)?;
            Ok((x1, x2))
        };

        let (q1, q2) = split_halves(&q.contiguous()?)?;
        let (k1, k2) = split_halves(&k.contiguous()?)?;

        let q1p = (q1.broadcast_mul(&cos)? - q2.broadcast_mul(&sin)?)?;
        let q2p = (q2.broadcast_mul(&cos)? + q1.broadcast_mul(&sin)?)?;
        let kp1 = (k1.broadcast_mul(&cos)? - k2.broadcast_mul(&sin)?)?;
        let kp2 = (k2.broadcast_mul(&cos)? + k1.broadcast_mul(&sin)?)?;

        let q_rot = Tensor::cat(&[q1p, q2p], candle_core::D::Minus1)?;
        let k_rot = Tensor::cat(&[kp1, kp2], candle_core::D::Minus1)?;
        Ok((q_rot, k_rot))
    }
}
...
impl TextAttention {
    ...
    fn forward(
        ...
        let k = if repeat_count > 1 {
            candle_transformers::utils::repeat_kv(k, repeat_count)?
        } else {
            k
        };
        let v = if repeat_count > 1 {
            candle_transformers::utils::repeat_kv(v, repeat_count)?
        } else {
            v
        };
...
impl Qwen2_5VLForConditionalGeneration {
    ...
    /// Forward that returns the final hidden states (after norm) without projecting to logits.
    /// Useful for feature extraction or custom heads (e.g., reranking MLP).
    pub fn forward_hidden(
        ...
        // Prepare input embeddings with optional vision tokens
        let input_embeds = if let Some(pixel_values) = pixel_values {
            let image_embeds = self.model.encode_images(pixel_values)?;
            let image_token_mask = input_ids.eq(self.model.config.image_token_id as i64)?;
            let text_embeds = self.model.text_model.embed_tokens.forward(input_ids)?;
            self.model
                .merge_vision_text_embeds(&text_embeds, &image_embeds, &image_token_mask)?
        } else {
        ...
        self.model.text_model.norm.forward(&hidden_states)
    }

...
pub static GLOBAL_RERANKER: Lazy<std::sync::Arc<tokio::sync::Mutex<Option<JinaRerankerEngine>>>> = Lazy::new(|| std::sync::Arc::new(tokio::sync::Mutex::new(None)));
...
pub fn map_reranker_key_to_repo(key: &str) -> String {
    ...
    if key.contains('/') { return key.to_string(); }
    let repo = match key {
...
impl JinaRerankerEngine {
    pub fn new(model_key: &str) -> Result<Self> {
        ...
        if let Some(w) = model_file.as_ref() {
            ...
            let _ = unsafe { crate::ai::hf::with_mmap_varbuilder_single(w, dtype, &device, |vb| {
            ...
            if let Some(idx) = index {
                ...
                let _ = unsafe { crate::ai::hf::with_mmap_varbuilder_multi(&files, dtype, &device, |vb| {
                    let _ = vb; // placeholder
                    Ok(())
                }) }?;
            } else {
                log::warn!("[Reranker] No model weights found in repo {hf_repo}; engine will be inert until implemented");
            }
        }
        // Build our local JinaM0Engine wrapper
        let engine = JinaM0Engine::load_from_repo(&hf_repo).ok();
        Ok(Self { tokenizer, device, engine })
    }
}

impl HFAiModel for JinaRerankerEngine {
    fn load_from_hub(model_key: &str) -> anyhow::Result<Self> { Self::new(model_key) }
}

pub async fn ensure_reranker_from_settings() -> anyhow::Result<()> {
    // Fast check without long work while holding the lock
    if GLOBAL_RERANKER.lock().await.is_some() { return Ok(()); }
    ...
    let model_key = if let Some(s) = cached.and_then(|c| c.reranker_model.clone()) {
        ...
        match crate::database::get_settings().await {
            Ok(s) => s.reranker_model.unwrap_or_else(|| "jinaai/jina-reranker-m0".to_string()),
    ...
    crate::ui::status::RERANK_STATUS.set_state(crate::ui::status::StatusState::Initializing, "Loading reranker");
    let res = JinaRerankerEngine::new(&model_key);
    match res {
        Ok(engine) => {
            let mut guard = GLOBAL_RERANKER.lock().await;
            *guard = Some(engine);
            crate::ui::status::RERANK_STATUS.set_state(crate::ui::status::StatusState::Running, "Ready");
            Ok(())
        }
        Err(e) => {
...
pub async fn jina_score_text_pairs(pairs: &[(String, String)]) -> anyhow::Result<Vec<f32>> {
    ...
    if GLOBAL_RERANKER.lock().await.is_none() { ensure_reranker_from_settings().await?; }
    let guard = GLOBAL_RERANKER.lock().await;
    let engine = guard.as_ref().ok_or_else(|| anyhow::anyhow!("Reranker not initialized"))?;
    ...
    if let Some(j) = &engine.engine {
        crate::ui::status::RERANK_STATUS.set_state(crate::ui::status::StatusState::Running, format!("Scoring {} pairs", pairs.len()));
        crate::ui::status::RERANK_STATUS.set_detail(format!("Batch {} len {}", pairs.len(), max_len));
        let res = j.compute_score_text(pairs, max_len);
        match res {
            Ok(v) => { crate::ui::status::RERANK_STATUS.set_state(crate::ui::status::StatusState::Idle, "Ready"); Ok(v) }
            Err(e) => { crate::ui::status::RERANK_STATUS.set_error(format!("Scoring failed: {e}")); Err(e) }
        }
    } else {
...
pub fn jina_score_text_pairs_blocking(pairs: &[(String, String)]) -> Vec<f32> {
    ...
    if let Ok(handle) = tokio::runtime::Handle::try_current() {
        return tokio::task::block_in_place(|| match handle.block_on(jina_score_text_pairs(pairs)) {
            Ok(v) => v,
            Err(_) => Vec::new(),
        });
    } else {

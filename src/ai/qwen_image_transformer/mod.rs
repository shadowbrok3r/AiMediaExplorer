use candle_core::{Result, Tensor, Device, Module, D};
use candle_nn::{VarBuilder, Conv2d, Conv2dConfig, conv2d, LayerNorm};
...
impl QwenImageTransformer2DModel {
    pub fn new(config: &QwenImageTransformerConfig, vb: VarBuilder) -> Result<Self> {
        ...
        for i in 0..config.num_layers {
            ...
            let q_proj = linear_no_bias(model_dim, model_dim, vb.pp(format!("layers.{i}.attn.q_proj")))?;
            let k_proj = linear_no_bias(config.joint_attention_dim, model_dim, vb.pp(format!("layers.{i}.attn.k_proj")))?;
            let v_proj = linear_no_bias(config.joint_attention_dim, model_dim, vb.pp(format!("layers.{i}.attn.v_proj")))?;
            let o_proj = linear_no_bias(model_dim, model_dim, vb.pp(format!("layers.{i}.attn.o_proj")))?;
            ...
            blocks.push(Block { ln1, q_proj, k_proj, v_proj, o_proj, ln2, fc1, fc2 });
        }
        Ok(Self { config: config.clone(), device: vb.device().clone(), proj_in, proj_out, blocks })
    }
    ...
    pub fn forward(&self, x: &Tensor, text: &Tensor) -> Result<Tensor> {
        ...
        // Apply each block
        let (bt, tlen, td) = text.dims3()?; assert_eq!(bt, b, "batch mismatch"); assert_eq!(td, self.config.joint_attention_dim, "text dim mismatch");
        for blk in &self.blocks {
            // Cross-attention to text tokens
            let residual = y.clone();
            let y_ln = blk.ln1.forward(&y)?; // [B,N,D]
            let q = blk.q_proj.forward(&y_ln)? // [B,N,D]
                .reshape((b, n, heads, head_dim))? // [B,N,H,Hd]
                .transpose(1, 2)?; // [B,H,N,Hd]
            let k = blk.k_proj.forward(text)? // [B,T,D]
                .reshape((b, tlen, heads, head_dim))?
                .transpose(1, 2)?; // [B,H,T,Hd]
            let v = blk.v_proj.forward(text)?
                .reshape((b, tlen, heads, head_dim))?
                .transpose(1, 2)?; // [B,H,T,Hd]

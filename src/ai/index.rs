    ...
    pub(crate) async fn index_file_internal(
        ...
        if !force {
            if let Some(existing) = self.get_file_metadata(&metadata.path).await {
                if existing.hash.is_some() && existing.hash == metadata.hash {
                    ...
                    let auto_clip = self.auto_clip_enabled.load(std::sync::atomic::Ordering::Relaxed);
                    let can_skip = if metadata.file_type != "image" { true } else {
                    ...
                    if can_skip {
                        ...
                        if metadata.file_type == "image" && auto_clip && existing.clip_embedding.is_none() {
        ...
        if metadata.file_type == "image" && path.exists() {
            ...
            if auto_desc {
                ...
                log::info!("[AI] Scheduling async vision description for {} (post-insert)", metadata.path);
                tokio::spawn(async move { arc_self.spawn_generate_vision_description(schedule_path); });
            }
            if auto_clip {

...
impl JinaM0Engine {
        ...
        match &self.weights {
            ...
            Some(Weights::Sharded(files)) => {
                ...
                for f in files {
                    ...
                    let st = SafeTensors::deserialize(&bytes)?;
                    if let Some(b) = scan_st(&st) {
                        best.get_or_insert(b);
                    }
                }
                Ok(best)
            }
    ...
    pub fn load_from_repo(repo: &str) -> Result<Self> {
        ...
        let (config, legacy_ids) = match mdl.get("config.json") {
            Ok(p) => {
                ...
                let full = Self::parse_qwen_config_from_bytes(&bytes)
                    .map_err(|e| {
        ...
        if let Ok(w) = mdl.get("model.safetensors") {
            ...
            unsafe {
                crate::ai::hf::with_mmap_varbuilder_single(&w, dtype, &device, |_vb| Ok(()))
            }?;
            ...
            unsafe {
                crate::ai::hf::with_mmap_varbuilder_multi(&files, dtype, &device, |_vb| Ok(()))
            }?;
        ...
        if let Some(cfg) = &config {
            ...
            log::warn!(
                "[Rerank/Jina] No config.json found or failed to parse; will try to infer from weights"
            );
        }
        Ok(Self {
            tokenizer,
    ...
    fn parse_qwen_config_from_bytes(bytes: &[u8]) -> Result<crate::ai::qwen2_5_vl::Config> {
        ...
        if let Ok(root) = serde_json::from_slice::<HFConfigRoot>(bytes) {
            if let Some(t) = root.text_config {
                let text_cfg = crate::ai::qwen2_5_vl::TextConfig {
                    ...
                    max_window_layers: t
                        .max_window_layers
                        .unwrap_or_else(|| t.num_hidden_layers.unwrap_or(28)),
                    mrope_section: None,
                    rope_scaling: None,
                };
                let vraw = root.vision_config.unwrap_or(HFVisionConfig {
                    hidden_size: None,
                    in_chans: None,
        ...
        let vision_start_token_id = v
            .get("vision_start_token_id")
            .and_then(|x| x.as_u64())
            .map(|u| u as u32)
            .unwrap_or(151652);
        let vision_end_token_id = v
    ...
    pub fn compute_score_text(
        ...
        log::info!(
            "[Rerank/Jina] Scoring {} pairs (seq_len={} max_length={})",
            b,
            l,
            max_length
        );
        ...
        let tconf = if let Some(cfg) = &self.config {
            cfg.text_config.clone()
        } else {
            match &self.weights {
                Some(Weights::Single(w)) => Self::discover_text_config_from_file(w)?
                    .ok_or_else(|| anyhow!("Failed to infer text config from single weights"))?,
                Some(Weights::Sharded(files)) => Self::discover_text_config_from_multi(files)?
                    .ok_or_else(|| anyhow!("Failed to infer text config from sharded weights"))?,
                None => return Err(anyhow!("Missing model weights (safetensors)")),
            }
        };
        let text_prefix = self
            .discover_text_prefix_from_weights()?
            .unwrap_or_default();
        log::info!(
            "[Rerank/Jina] Using text prefix: '{}'",
            ...
            Some(Weights::Single(w)) => {
                let full = unsafe {
                    crate::ai::hf::with_mmap_varbuilder_single(w, self.dtype, &self.device, |vb| {
                        let vb = if text_prefix.is_empty() {
                            vb
                        } else {
                            vb.pp(&text_prefix)
                        };
                        crate::ai::qwen2_5_vl::TextModel::new(&tconf, vb)
                ...
                match full {
                    Ok(mut model) => {
                        match self.forward_text_hidden(&mut model, &input_ids) {
                            ...
                            Err(e) => {
                                log::warn!(
                                    "[Rerank/Jina] Text forward failed: {e:?}. Falling back to embed-only pooling."
                                );
                                self.embed_only_pooled(
                                    &input_ids,
                                    &attn_mask_ids,
                                    &tconf,
                                    &text_prefix,
                    ...
                    Err(e) => {
                        log::warn!(
                            "[Rerank/Jina] TextModel build failed: {e:?}. Falling back to embed-only pooling."
                        );
        ...
        log::info!(
            "[Rerank/Jina] Pooled hidden dim: {}",
            pooled.dim(1).unwrap_or(0)
        );
            ...
            if let Some(base) = self.discover_head_mlp_base_from_weights()? {
                ...
                match &self.weights {
                    Some(Weights::Single(w)) => unsafe {
                        let head = crate::ai::hf::with_mmap_varbuilder_single(
                            w,
                            self.dtype,
                            &self.device,
                            |vb| {
                    ...
                    Some(Weights::Sharded(files)) => unsafe {
                        let head = crate::ai::hf::with_mmap_varbuilder_multi(
                            files,
                            self.dtype,
                            &self.device,
                            |vb| {
                ...
                match &self.weights {
                    Some(Weights::Single(_)) => {
                        if let Some(head) = self.load_scoring_head_linear_single(
                            match &self.weights {
                                Some(Weights::Single(w)) => w,
                                _ => unreachable!(),
                            },
                            hidden_size,
                        )? {
                            ...
                            log::warn!(
                                "[Rerank/Jina] Ranking head weights not found (linear, single). Dumping head-like keys…"
                            );
                            self.debug_log_head_candidates(12);
                            return Err(anyhow!(
                                "Ranking head weights not found (linear, single)."
                            ));
                        }
                    }
                    Some(Weights::Sharded(files)) => {
                        if let Some(head) =
                            self.load_scoring_head_linear_sharded(files, hidden_size)?
                        {
                            log::info!("[Rerank/Jina] Using linear head (sharded)");
                            head.forward(&pooled)?
                        } else {
                            log::warn!(
                                "[Rerank/Jina] Ranking head weights not found (linear, sharded). Dumping head-like keys…"
                            );
                            self.debug_log_head_candidates(12);
                            return Err(anyhow!(
                                "Ranking head weights not found (linear, sharded)."
                            ));
                        }
                    }
                    None => unreachable!(),
        ...
        log::info!(
            "[Rerank/Jina] Raw logits len: {} (batch {})",
            scores.len(),
            bsz
        );
        for s in &mut scores {
        ...
        if scores.len() != bsz {
            log::error!(
                "[Rerank/Jina] Score length mismatch: got {} expected {}",
                scores.len(),
                bsz
            );
    ...
    fn prepare_inputs(
        ...
        for (q, d) in pairs.iter() {
            let enc = self
                .tokenizer
                .encode(
                    EncodeInput::Dual(q.as_str().into(), d.as_str().into()),
                    true,
                )
    ...
    fn load_scoring_head_linear_single(
        &self,
        _w: &PathBuf,
        hidden_size: usize,
    ) -> Result<Option<Linear>> {
        self.load_scoring_head_linear_inner(hidden_size, false)
    }

    fn load_scoring_head_linear_sharded(
        &self,
        _files: &[PathBuf],
        hidden_size: usize,
    ) -> Result<Option<Linear>> {
        self.load_scoring_head_linear_inner(hidden_size, true)
    }

    fn load_scoring_head_linear_inner(
        &self,
        hidden_size: usize,
        sharded: bool,
    ) -> Result<Option<Linear>> {
        ...
        let try_build_with_prefix =
            |vb: candle_nn::VarBuilder, prefix: &str| -> Result<Option<Linear>> {
                if let Ok(l) = linear_no_bias(hidden_size, 1, vb.pp(prefix)) {
                    return Ok(Some(l));
                }
                if let Ok(l) = linear(hidden_size, 1, vb.pp(prefix)) {
                    return Ok(Some(l));
                }
                Ok(None)
            };
        ...
        match (&self.weights, sharded) {
            (Some(Weights::Single(w)), false) => unsafe {
                if let Some(lin) =
                    crate::ai::hf::with_mmap_varbuilder_single(w, self.dtype, &self.device, |vb| {
                        try_build(vb, &candidates)
                    })?
                {
                    return Ok(Some(lin));
                }
                if let Some(prefix) = Self::discover_head_prefix_from_file(w)? {
                    return crate::ai::hf::with_mmap_varbuilder_single(
                        w,
                        self.dtype,
                        &self.device,
                        |vb| try_build_with_prefix(vb, &prefix),
                    );
                }
                Ok(None)
            },
    ...
    fn discover_head_prefix_from_file(path: &PathBuf) -> Result<Option<String>> {
        ...
        for name in st.names() {
            ...
            if lname.contains("rank") {
                score += 5;
            }
            if lname.contains("rerank") {
                score += 5;
            }
            if lname.contains("score") {
                score += 4;
            }
            if lname.contains("head") {
                score += 3;
            }
            if lname.contains("cls") {
                score += 1;
            }
    ...
    fn debug_log_head_candidates(&self, limit: usize) {
                ...
                if let Ok(bytes) = std::fs::read(w) {
                    if let Ok(st) = SafeTensors::deserialize(&bytes) {
                        for name in st.names() {
                            ...
                            if ln.ends_with(".weight")
                                && (ln.contains("rank")
                                    || ln.contains("score")
                                    || ln.contains("head"))
                            {
                                if logged < limit {
                                    log::info!("[Rerank/Jina] head-candidate: {name}");
                                    logged += 1;
                                }
                                if logged >= limit {
                                    break;
        ...
        if logged == 0 {
            log::info!(
                "[Rerank/Jina] No head-like keys found in weights (scanned up to {limit} entries per shard)"
            );
        }
    }
}
...
impl JinaM0Engine {
    ...
    fn embed_only_pooled(
        ...
        let pooled = match &self.weights {
            Some(Weights::Single(w)) => unsafe {
                let (emb, norm_w) = crate::ai::hf::with_mmap_varbuilder_single(
                    w,
                    self.dtype,
                    &self.device,
                    |vb| {
            ...
            Some(Weights::Sharded(files)) => unsafe {
                let (emb, norm_w) = crate::ai::hf::with_mmap_varbuilder_multi(
                    files,
                    self.dtype,
                    &self.device,
                    |vb| {
    ...
    fn discover_text_config_from_file(
        path: &PathBuf,
    ) -> Result<Option<crate::ai::qwen2_5_vl::TextConfig>> {
        let bytes = std::fs::read(path)?;
        let st = SafeTensors::deserialize(&bytes)?;
        Self::discover_text_config_from_st(&st)
    }

    fn discover_text_config_from_multi(
        files: &[PathBuf],
    ) -> Result<Option<crate::ai::qwen2_5_vl::TextConfig>> {
    ...
    fn discover_text_config_from_st(
        ...
        let (vocab_size, hidden_size) = match embed_shape {
            Some(s) => s,
            None => return Ok(None),
        };
        ...
        for name in st.names() {
            if name.contains("layers.0.mlp.gate_proj.weight") {
                ...
                let shape = t.shape();
                if shape.len() == 2 {
                    intermediate_size = Some(shape[0] as usize);
                    break;
                }
            }
        }
        ...
        for name in st.names() {
            if let Some(idx) = name.find("layers.") {
                ...
                if let Some(end) = rest.find('.') {
                    if let Ok(i) = rest[..end].parse::<usize>() {
                        if i > max_layer {
                            max_layer = i;
                        }
                    }
        ...
        let (num_attention_heads, num_key_value_heads) = best.unwrap_or_else(|| {
            ...
            let nh = hidden_size / head_dim;
            let kvh = (kv_out / head_dim).max(2).min(nh.max(2));
            (nh.max(1), kvh)
        });
...
impl RawJinaConfig {
    fn into_qwen_config(self) -> crate::ai::qwen2_5_vl::Config {
        ...
        let vraw = self.vision_config.unwrap_or(RawVisionConfig {
            hidden_size: None,
            in_chans: None,
            spatial_patch_size: None,
        });

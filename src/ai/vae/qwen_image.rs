...
impl QwenImageVaeSimplified {
    ...
    pub fn new_with_hints(
        ...
        Ok(Self {
            ...
            enc_in,
            enc_downs,
            enc_extra: enc_extra_layers,
            enc_out,
            dec_in,
            dec_ups,
            dec_extra: dec_extra_layers,
            dec_out,
        })
    }
    ...
    pub fn encode(&self, x: &Tensor, deterministic: bool) -> Result<Tensor> {
        ...
        for d in &self.enc_downs {
            y = d.forward(&y)?;
        }
        for d in &self.enc_extra {
            y = d.forward(&y)?;
        }
...
impl QwenImageVaeSimplified {
    pub fn downsample_factor(&self) -> usize {
        1usize << self.enc_downs.len().max(1)
    }

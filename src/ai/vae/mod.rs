use candle_core::{Result, Tensor, DType, Device, Module, IndexOp};
use candle_nn::{Conv2d, Conv2dConfig, ConvTranspose2d, ConvTranspose2dConfig, VarBuilder, conv2d, conv_transpose2d};
...
pub struct VaeConfig {
    ...
    #[serde(default = "default_block_out_channels")] pub block_out_channels: Vec<usize>,
    #[serde(default = "default_down_block_types")] pub down_block_types: Vec<String>,
    #[serde(default = "default_up_block_types")] pub up_block_types: Vec<String>,
    #[serde(default)] pub sample_size: Option<usize>,
    #[serde(default)] pub scaling_factor: Option<f32>,
}

fn default_block_out_channels() -> Vec<usize> { vec![128, 256, 512, 512] }
fn default_down_block_types() -> Vec<String> { vec!["DownEncoderBlock2D".into(); 4] }
fn default_up_block_types() -> Vec<String> { vec!["UpDecoderBlock2D".into(); 4] }
...
impl ResBlock2D {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let residual = if let Some(s) = &self.skip { s.forward(x)? } else { x.clone() };
...
impl AutoencoderKL {
    ...
    let chs: Vec<usize> = config.block_out_channels.clone();
        ...
        if chs.len() >= 2 {
            ...
            for i in (1..chs.len()).rev() {
                ...
                let res = ResBlock2D::new(out_ch, out_ch, vb.pp("decoder"), &format!("res.{}", chs.len() - 1 - i))?;
                up_res.push(res);
            }
        }
        ...
        Ok(Self { config: config.clone(), enc_in, down_blocks, down_res, to_mu, to_logvar, from_latent, up_blocks, up_res, dec_out, device: dev })
    }
    ...
    let _bchw = x.dims4()?;
        ...
        for (i, conv) in self.down_blocks.iter().enumerate() {
            ...
            if let Some(rb) = self.down_res.get(i) { y = rb.forward(&y)?; }
        }
    ...
    pub fn encode_deterministic(&self, x: &Tensor) -> Result<Tensor> { self.encode(x, false) }

    // Levels equals number of stride-2 downsamples
    pub fn levels(&self) -> usize { self.down_blocks.len().max(1) }

    pub fn downsample_factor(&self) -> usize { 1usize << self.levels() }
    ...
    pub fn decode(&self, z: &Tensor, h: usize, w: usize) -> Result<Tensor> {
        ...
        for (i, deconv) in self.up_blocks.iter().enumerate() {
            ...
            if let Some(rb) = self.up_res.get(i) { y = rb.forward(&y)?; }
        }
...
pub trait VaeLike: Send + Sync {
    fn encode_with_auto_pad(&self, x: &Tensor, sample: bool) -> Result<(Tensor, usize, usize, usize, usize)>;
...
impl VaeLike for AutoencoderKL {
    ...
    fn downsample_factor(&self) -> usize { Self::downsample_factor(self) }
}

pub fn build_vae_from_files(files: &[std::path::PathBuf], dtype: DType, device: &Device, cfg: &VaeConfig) -> anyhow::Result<AutoencoderKL> {
...
pub fn build_vae_from_files_with_5d_squeeze(files: &[std::path::PathBuf], dtype: DType, device: &Device, cfg: &VaeConfig) -> anyhow::Result<AutoencoderKL> {
    ...
    for f in files.iter() {
        let bytes = match std::fs::read(f) { Ok(b) => b, Err(_) => continue };
        let st = match SafeTensors::deserialize(&bytes) { Ok(s) => s, Err(_) => continue };
        for k in needed.iter() {
            ...
            if let Ok(t) = st.tensor(k) {
                ...
                let el = if total_elems == 0 { 1 } else { data.len().saturating_div(total_elems) };
                if shape.len() == 5 {
                    ...
                    for oo in 0..o { for ii in 0..i { for hh in 0..kh { for ww in 0..kw {
                        let src_index = ((((oo * i + ii) * tdim + tm) * kh + hh) * kw + ww) * el;
                        let src_byte_off = src_index;
                        let dst_byte_off = dst_idx * el;
                        out[dst_byte_off..dst_byte_off+el].copy_from_slice(&data[src_byte_off..src_byte_off+el]);
                        dst_idx += 1;
                    }}}}
                    collected.push((k.clone(), dt, vec![o, i, kh, kw], out));
                    have.insert(k.clone());
                } else if shape.len() == 4 || shape.len() == 1 { // conv weight or bias
                    let out = data.to_vec();
    ...
    if have.len() != needed.len() {
        let missing: Vec<_> = needed.iter().filter(|k| !have.contains(*k)).cloned().collect();
    ...
    for (i, (k, _, _, _)) in collected.iter().enumerate() { idx.insert(k.as_str(), i); }
    ...
    for k in ordered_keys.iter() {
        if let Some(&i) = idx.get(k.as_str()) {
            ...
            let tv = TensorView::new(*dt, shp.clone(), buf.as_slice()).map_err(anyhow::Error::msg)?;
    ...
    let vb = unsafe { VarBuilder::from_mmaped_safetensors(std::slice::from_ref(&tmp_path_buf), dtype, device)? };
    AutoencoderKL::new(cfg, vb).map_err(anyhow::Error::from)
}
...
pub struct VaeConfigCompatQwen {
    #[serde(default)] _class_name: Option<String>,
    #[serde(default)] base_dim: Option<usize>,
    #[serde(default)] dim_mult: Option<Vec<usize>>, // e.g., [1,2,4,4]
    #[serde(default)] z_dim: Option<usize>,
}
...
pub fn build_qwen_vae_simplified_from_files(files: &[std::path::PathBuf], dtype: DType, device: &Device, scaling: f32) -> anyhow::Result<QwenImageVaeSimplified> {
    ...
    if collected.is_empty() { anyhow::bail!("No matching Qwen VAE weights found to adapt."); }
    ...
    for (name, dt, shape, buf) in collected.iter() {
        ...
        if name == "encoder.conv_in.weight" && (shape.len() == 4) {
        ...
        } else if name.contains("encoder.down_blocks.") && (name.contains("conv1.weight") || name.contains("conv2.weight") || name.contains("conv_shortcut.weight")) && shape.len() == 4 {
            // Adapter convs inside resnets/shortcuts
            let k = shape.get(2).copied().unwrap_or(3);
            enc_extra.push((name.clone(), shape[1], shape[0], k));
        ...
        } else if name.contains("decoder.up_blocks.") && (name.contains("conv1.weight") || name.contains("conv2.weight") || name.contains("conv_shortcut.weight") || name.contains("time_conv.weight")) && (shape.len() == 4 || shape.len() == 5) {
            ...
            dec_extra.push((name.clone(), shape[1], shape[0], k));
        }
    }
    let bin = serialize(ordered.into_iter(), None).map_err(anyhow::Error::msg)?;
    let tmp = tempfile::NamedTempFile::new()?;
    ...
    unsafe {
        crate::ai::hf::with_mmap_varbuilder_multi(std::slice::from_ref(&tmp_path_buf), dtype, device, move |vb| {
            // Use hints if available, else fallback to heuristic defaults
            if let (Some(ein), Some(eout), Some(din), Some(dout)) = (enc_in_ch, enc_out_ch, dec_in_ch, dec_out_ch) {
                ...
                let enc_use = if enc_chain.is_empty() { &enc_down_shapes[..] } else { &enc_chain[..] };
                let dec_use = if dec_chain.is_empty() { &dec_up_shapes[..] } else { &dec_chain[..] };

                QwenImageVaeSimplified::new_with_hints(vb, scaling, ein, enc_use, eout, din, dec_use, dout, &enc_extra_owned)

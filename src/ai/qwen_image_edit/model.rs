...
use crate::ai::vae::{VaeConfig as AEKLConfig, VaeLike, build_vae_from_files, build_vae_from_files_with_5d_squeeze, build_qwen_vae_simplified_from_files};
...
impl QwenImageEditPipeline {
        ...
        for (i, f) in files.iter().take(3).enumerate() {
            match std::fs::read(f) {
                Ok(bytes) => match SafeTensors::deserialize(&bytes) {
                    Ok(st) => {
                        let keys = st.names();
                        log::info!("[qwen-image-edit] {label} diag: file#{i} {} keys", keys.len());
                        ...
                        if label.contains("transformer") {
                            ...
                            for k in keys.iter().filter(|n| n.contains("patch") || n.contains("embed") || n.contains("proj")) {
                                log::info!("  hint: {}", k);
                            }
                        ...
                        for k in expected_keys {
                            ...
                            if found {
                                ...
                                if let Some(name) = keys.iter().find(|n| **n == *k)
                        ...
                        if label.contains("vae") {
                            ...
                            for name in keys.iter().filter(|n| n.starts_with("encoder") || n.starts_with("decoder")) {
                                if let Ok(t) = st.tensor(*name) {
                                    let shape: Vec<_> = t.shape().into();
                                    log::info!("   {} -> {:?}", name, shape);
                                    count += 1;
                    ...
                    Err(e) => log::warn!("[qwen-image-edit] {label} diag: cannot parse safetensors {}: {e}", f.display()),
                },
                Err(e) => log::warn!("[qwen-image-edit] {label} diag: cannot read {}: {e}", f.display()),
            }
        }
    }
    pub fn load_from_hf(repo_id: &str, prefer_dtype: DType) -> Result<Self> {
        ...
        let tokenizer_json = {
            ...
            for c in candidates.iter() {
                if let Ok(p) = repo.get(c) { found = Some(p); break; }
            }
            ...
            if let Ok(s) = std::fs::read_to_string(&idx_path) {
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(&s) {
                    ...
                    if let Some(map) = v.get("weight_map").and_then(|m| m.as_object()) {
                        for fname in map.values().filter_map(|v| v.as_str()) { uniq.insert(fname.to_string()); }
                    }
                    let mut list: Vec<String> = uniq.into_iter().collect();
                    list.sort();
                    for f in list.into_iter() {
                        let full = format!("text_encoder/{}", f);
                        if let Ok(p) = repo.get(&full) { text_encoder_files.push(p); }
                    }
                }
            }
        }
            ...
            if let Ok(s) = std::fs::read_to_string(&idx_path) {
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(&s) {
                    ...
                    for f in list.into_iter() {
                        ...
                        if let Ok(p) = repo.get(&full) { transformer_files.push(p); }
                    }
                }
            }
        }
        ...
        let text_encoder_config_json = if let Some(p) = &text_encoder_config { std::fs::read_to_string(p).ok().and_then(|s| serde_json::from_str(&s).ok()) } else { None };
    let transformer_config_json = if let Some(p) = &transformer_config { std::fs::read_to_string(p).ok().and_then(|s| serde_json::from_str::<serde_json::Value>(&s).ok()) } else { None };
    let vae_config_json = if let Some(p) = &vae_config { std::fs::read_to_string(p).ok().and_then(|s| serde_json::from_str::<serde_json::Value>(&s).ok()) } else { None };
        ...
        // Try to build VAE
        let vae_cfg = if let Some(v) = &vae_config_json { serde_json::from_value::<AEKLConfig>(v.clone()).ok() } else { None };
    let vae = if let Some(cfg) = &vae_cfg { build_vae_from_files(&vae_files, dtype, &device, cfg).ok().map(|v| Box::new(v) as Box<dyn VaeLike>) } else { None };
        ...
        let transformer_model = if let Some(tr_json) = &transformer_config_json {
            match serde_json::from_value::<QwenImageTransformerConfig>(tr_json.clone()) {
                Ok(cfg) => {
                    ...
                    let vb_root = match unsafe { candle_nn::VarBuilder::from_mmaped_safetensors(&transformer_files, dtype, &device) } {
                        Ok(vb) => vb,
                        Err(e) => {
                            log::error!("[qwen-image-edit] Failed to map transformer weights: {e}");
                            Self::diag_scan_weights(&transformer_files, "transformer", &[
                                "proj_in.weight",
                                "proj_out.weight",
                                "layers.0.ln1.weight",
                    ...
                    for r in roots.iter() {
                        ...
                        match QwenImageTransformer2DModel::new(&cfg, vb_try) {
                            Ok(m) => { built = Some(m); break; }
                            Err(e) => {
                                log::debug!("[qwen-image-edit] transformer build failed for root='{}': {}", r, e);
                                continue;
                            }
                        }
                    }
                    if built.is_none() {
                        log::error!("[qwen-image-edit] Failed to build transformer from weights across known prefixes.");
                        Self::diag_scan_weights(&transformer_files, "transformer", &[
                            "proj_in.weight",
                            "proj_out.weight",
                            "layers.0.ln1.weight",
                ...
                Err(e) => { log::error!("[qwen-image-edit] transformer config parse failed: {}", e); None }
            }
        } else { None };
                    ...
                    if let Some(first) = transformer_files.get(0) {
                        if let Ok(bytes) = std::fs::read(first) {
                            if let Ok(st) = safetensors::SafeTensors::deserialize(&bytes) {
                                looks_flux = st.names().iter().any(|k| k.starts_with("transformer_blocks."));
                            }
                        }
                    }
                    if looks_flux {
                        match FluxTransformer2DModel::new(&cfg_flux, unsafe { candle_nn::VarBuilder::from_mmaped_safetensors(&transformer_files, dtype, &device)? }) {
                            Ok(m) => Some(m),
                            Err(e) => { log::error!("[qwen-image-edit] Flux transformer build failed: {}", e); None }
                        }
                    } else { None }
                }
                Err(_) => None,
    ...
    pub fn info(&self) {
        ...
        let opt = |label: &str, p: &Option<std::path::PathBuf>| {
            if let Some(p) = p { log::info!("  {}: {}", label, p.display()); }
        };
        opt("processor.preprocessor_config", &self.paths.processor_preprocessor_config);
        opt("processor.tokenizer_config", &self.paths.processor_tokenizer_config);
        opt("processor.tokenizer.json", &self.paths.processor_tokenizer_json);
        ...
        if let Some(te) = &self.text_encoder_config_json { if let Some(class) = te.get("_class_name").and_then(|v| v.as_str()) { log::info!(" text_encoder._class_name={}", class); } }
        if let Some(tr) = &self.transformer_config_json { if let Some(class) = tr.get("_class_name").and_then(|v| v.as_str()) { log::info!(" transformer._class_name={}", class); } }
        if let Some(va) = &self.vae_config_json { if let Some(class) = va.get("_class_name").and_then(|v| v.as_str()) { log::info!(" vae._class_name={}", class); } }
        if let Some(k) = &self.text_encoder_class { log::info!(" text_encoder.kind={:?}", k); }
        if let Some(k) = &self.transformer_class { log::info!(" transformer.kind={:?}", k); }
    }

    // Denoiser scaffold: encode -> noise -> steps -> decode. Returns PNG bytes.
    pub fn denoise_loop(&self, image_path: &std::path::Path, opts: &crate::ai::qwen_image_edit::EditOptions) -> Result<Vec<u8>> {
        // Validate components
        ...
        let vae: &dyn VaeLike = match self.vae.as_ref() {
            ...
            None => {
                ...
                let cfg_opt: Option<AEKLConfig> = if let Some(vj) = &self.vae_config_json {
                    serde_json::from_value::<AEKLConfig>(vj.clone()).ok()
                        .or_else(|| serde_json::from_value::<crate::ai::vae::VaeConfigCompatQwen>(vj.clone()).ok().map(|c| c.to_internal()))
                } else if let Some(p) = &self.paths.vae_config {
                    std::fs::read_to_string(p).ok().and_then(|s| serde_json::from_str::<AEKLConfig>(&s).ok())
                        .or_else(|| std::fs::read_to_string(p).ok().and_then(|s| serde_json::from_str::<crate::ai::vae::VaeConfigCompatQwen>(&s).ok()).map(|c| c.to_internal()))
                } else { None };
                if let Some(mut cfg) = cfg_opt {
                    // Ensure required fields are present even if config is minimal
                    if cfg.in_channels == 0 { cfg.in_channels = 3; }
                    if cfg.out_channels == 0 { cfg.out_channels = 3; }
                    if cfg.latent_channels == 0 { cfg.latent_channels = 16; }
                    match build_vae_from_files(&self.paths.vae_files, self.dtype, &self.device, &cfg) {
                        Ok(v) => { vae_local = Box::new(v) as Box<dyn VaeLike>; &*vae_local }
                        Err(e) => {
                            log::error!("[qwen-image-edit] VAE strict load failed: {e}");
                            Self::diag_scan_weights(&self.paths.vae_files, "vae", &[
                            ...
                            match build_vae_from_files_with_5d_squeeze(&self.paths.vae_files, self.dtype, &self.device, &cfg) {
                                Ok(v) => { log::warn!("[qwen-image-edit] VAE 5D->4D squeeze applied."); vae_local = Box::new(v) as Box<dyn VaeLike>; &*vae_local }
                                Err(e2) => {
                                    log::warn!("[qwen-image-edit] VAE squeeze failed: {e2}; trying Qwen-native simplified VAE mapping.");
                                    ...
                                    match build_qwen_vae_simplified_from_files(&self.paths.vae_files, self.dtype, &self.device, scale) {
                                        Ok(q) => { vae_local = Box::new(q) as Box<dyn VaeLike>; &*vae_local }
                                        Err(e3) => { bail!("Failed to build any VAE (internal, 5D squeeze, qwen-native): {e3}"); }
                                    }
                                }
                            }
                        }
                    ...
                    bail!("VAE not loaded and failed to parse vae/config.json from repo (both internal and compatibility).");
                }
            }
        };
    ...
    if tr_flux_opt.is_none() && tr_qwen_opt.is_none() { bail!("Transformer not loaded"); }
        let sched = self.scheduler.as_ref().ok_or_else(|| anyhow::anyhow!("Scheduler missing"))?;
    ...
    if x.dtype() != self.dtype { x = x.to_dtype(self.dtype)?; }
        ...
        let (text_cond, text_uncond) = if self.tokenizer.is_some() && self.has_text_encoder {
            ...
            let (vocab_size, hidden_size, eps) = if let Some(cfg) = &self.text_encoder_config_json {
                ...
                let vs = tc.and_then(|t| t.get("vocab_size")).and_then(|v| v.as_u64()).unwrap_or(151936) as usize;
                // Determine joint text dim from whichever transformer is present
                let joint_dim = self.transformer_flux.as_ref().map(|m| m.config.joint_attention_dim)
                    .or_else(|| self.transformer_model.as_ref().map(|m| m.config.joint_attention_dim))
                    .unwrap_or(3584);
                let hs = tc.and_then(|t| t.get("hidden_size")).and_then(|v| v.as_u64()).unwrap_or(joint_dim as u64) as usize;
                let e = tc.and_then(|t| t.get("rms_norm_eps")).and_then(|v| v.as_f64()).unwrap_or(1e-6) as f32;
                (vs, hs, e)
            } else {
                let joint_dim = self.transformer_flux.as_ref().map(|m| m.config.joint_attention_dim)
                    .or_else(|| self.transformer_model.as_ref().map(|m| m.config.joint_attention_dim))
            ...
            let (emb_layer, norm_w) = unsafe {
                with_mmap_varbuilder_multi(&self.paths.text_encoder_files, self.dtype, &self.device, |vb: VarBuilder| {
                    ...
                    for p in attempts.iter() {
                        if let Ok(v) = try_build(vb.clone(), p) { return Ok(v); }
                    }
    ...
    let mut buf = image::ImageBuffer::<image::Rgb<u8>, Vec<u8>>::new(ww as u32, hh as u32);
        for y in 0..hh { for x in 0..ww {
            let r = img_out.i((0, y, x))?.to_scalar::<u8>()?;
            let g = img_out.i((1, y, x))?.to_scalar::<u8>()?;
            let b = img_out.i((2, y, x))?.to_scalar::<u8>()?;
            buf.put_pixel(x as u32, y as u32, image::Rgb([r, g, b]));
        }}
        let mut out_bytes = Vec::new();
        {
            let mut cur = std::io::Cursor::new(&mut out_bytes);
            image::DynamicImage::ImageRgb8(buf).write_to(&mut cur, image::ImageFormat::Png)?;
        }
        Ok(out_bytes)
    }
